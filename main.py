...
from keyboards import keyboard1, keyboardM, keyboardS, keyboardP, keyboardI
from keyboardshahar import keyboardbuxoro, keyboardandijon, keyboardfargona
from keyboardshahar import keyboardjizzax, keyboardsurxondaryo, keyboardnamangan, keyboardnavoiy, keyboardqashqadaryo, \
    keyboardsirdaryo
from keyboardshahar import keyboardsamarqand, keyboardqoraqalpogiston, keyboardxorazm, keyboardtoshkent
from config import API_TOKEN
from prayertimesapi import bugun, hozirgi, bomdod, quyosh, peshin, asr, shom, xufton
...
async def select_mintaqa(message: types.Message):
    await message.answer(
        text="Assalomu Alaykum\n<b>Ramadan Taqvimi</b> 2️⃣0️⃣2️⃣3️⃣ \n\n botiga xush kelibsiz\nKerakli bo'limni "
             "tanlang!",
        parse_mode='html',
...
async def send_zikrlar(message: types.Message):
    await message.answer(
        text=f"<b>OYATAL KURSIY</b>\n\n"
             ...
             f" Laa ta'xuzuhu cinatuv va laa navm. Lahuu maa fis-samaavaati va maa fil ard."
    ...
    await bot.send_audio(
        message.chat.id, open("G:/bot projects/RamadanTaqvim/images/Ayatul kursi duosi o'qilishi.mp3", "rb"),
        parse_mode='html',
        reply_markup=keyboardP
    )
...
async def send_bothaqaida(message: types.Message):
    await message.answer(
        text=f"👨🏻‍💻 Loyiha asoschisi  — Shahzod Rashidov\n"
             f"📜 Ma'lumotlar: \n "
             f"➖islom.uz \n"
             f"➖namozvaqti.uz\n "
             f"sahifalardan olindi \n"
             f"📩 Murojaatlar uchun — @Shahzod_0109",
        reply_markup=keyboardS
    )


@dp.message_handler(text='🕍Namaz vaqtlari')
async def send_bothaqaida(message: types.Message):
    await message.answer(
        text="Namoz vaqtlari🕍\n\n"
